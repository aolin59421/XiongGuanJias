...
@interface CPOCycleScrollView () <UIScrollViewDelegate>
...
@implementation CPOCycleScrollView
...
- (id)initWithFrame:(CGRect)frame pageContol:(NSInteger)page animationDuration:(NSTimeInterval)animationDuration
{
    ...
    self.pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(220, 90, 100, 100)];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if(contentOffsetX >= (2 * CGRectGetWidth(scrollView.frame))) {
        ...
        if ([_delegate respondsToSelector:@selector(page:)]) {
            [_delegate page:self.currentPageIndex];
        }
    }
    if(contentOffsetX <= 0) {
        self.currentPageIndex = [self getValidNextPageIndexWithPageIndex:self.currentPageIndex - 1];
