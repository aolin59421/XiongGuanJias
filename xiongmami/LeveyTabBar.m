...
- (id)initWithFrame:(CGRect)frame buttonImages:(NSArray *)imageArray
{
    ...
    if (self)
	{
		...
		UIButton *btn;
		CGFloat width = VIEW_FRAME_SIZE_WIDTH / [imageArray count];
		for (NSInteger i = 0; i < [imageArray count]; i++)
		{
            ...
            if (iPhone6Plus) {
//                NSLog(@"iphone6+ %f-%f",self.frame.size.width,self.frame.size.height);
                btn.frame = CGRectMake(width * i, 0, 138, 65);
            }
...
- (void)removeTabAtIndex:(NSInteger)index
{
    ...
    // Re-index the buttons
     CGFloat width = VIEW_FRAME_SIZE_WIDTH / [self.buttons count];
...
{
    // Re-index the buttons
    CGFloat width = VIEW_FRAME_SIZE_WIDTH / ([self.buttons count] + 1);
