...
- (id)initWithViewControllers:(NSArray *)vcs imageArray:(NSArray *)arr;
{
	...
	if (self != nil)
	{
		...
		_containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, VIEW_FRAME_SIZE_WIDTH, VIEW_FRAME_SIZE_HEIGHT)];
        ...
        if (iPhone6Plus) {
            _transitionView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, VIEW_FRAME_SIZE_WIDTH, _containerView.frame.size.height - 65)];
            _tabBar = [[LeveyTabBar alloc] initWithFrame:CGRectMake(0, _containerView.frame.size.height - 65, VIEW_FRAME_SIZE_WIDTH,65) buttonImages:arr];
        }else{
            _transitionView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, VIEW_FRAME_SIZE_WIDTH, _containerView.frame.size.height - kTabBarHeight)];
            _tabBar = [[LeveyTabBar alloc] initWithFrame:CGRectMake(0, _containerView.frame.size.height - kTabBarHeight, VIEW_FRAME_SIZE_WIDTH, kTabBarHeight) buttonImages:arr];
        }
...
- (void)setTabBarTransparent:(BOOL)yesOrNo
{
	...
	else
	{
        ...
        if (iPhone6Plus) {
            _transitionView.frame = CGRectMake(0, 0, VIEW_FRAME_SIZE_WIDTH, _containerView.frame.size.height - 65);
        }else{
            _transitionView.frame = CGRectMake(0, 0, VIEW_FRAME_SIZE_WIDTH, _containerView.frame.size.height - kTabBarHeight);
        }
	}
...
- (void)hidesTabBar:(BOOL)yesOrNO animated:(BOOL)animated;
{
	...
	else 
	{
        ...
        if (iPhone6Plus) {
            if (self.tabBar.frame.origin.y == self.view.frame.size.height - 65)
            {
                return;
            }
        }else if (self.tabBar.frame.origin.y == self.view.frame.size.height - kTabBarHeight)
        {
            return;
        }
	}
	...
	if (animated == YES)
	{
		...
		if (yesOrNO == YES)
		{
            ...
            if (iPhone6Plus) {
                self.tabBar.frame = CGRectMake(self.tabBar.frame.origin.x, self.tabBar.frame.origin.y + 65, self.tabBar.frame.size.width, self.tabBar.frame.size.height);
            }else{
